...
export const selectFilterReleaseIdsForCurrentBoard = createSelector(
  orm,
  (state) => selectPath(state).boardId,
  ({ Board }, id) => {
    if (!id) {
    ...
    return boardModel.filterReleases.toRefArray().map((release) => release.id);
  },
);
...
export const selectEpicCardsForCurrentBoard = createSelector(
  orm,
  (state) => selectPath(state).boardId,
  ({ Board }, id) => {
    if (!id) {
    ...
    return boardModel
      .getCardsModelArray()
      .filter((cardModel) => cardModel.type === 'epic')
      .map((cardModel) => cardModel.ref);
  },
);
...
export default {
  ...
  selectFilterReleaseIdsForCurrentBoard,
